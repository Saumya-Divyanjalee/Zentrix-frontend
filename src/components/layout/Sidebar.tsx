...
import { LayoutDashboard, CheckSquare, FileText, Calendar, BarChart3, Sparkles, History, User, LogOut, GraduationCap, X } from 'lucide-react';
...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-screen w-60 bg-white border-r border-zinc-100 flex flex-col z-50 transition-transform duration-300 lg:translate-x-0 ${
          open ? 'translate-x-0' : '-translate-x-full'
        }`}
        style={{ boxShadow: '4px 0 24px rgba(99,102,241,0.04)' }}>

        <div className="px-5 pt-6 pb-5 border-b border-zinc-100 flex items-center justify-between">
          ...
          <button onClick={onClose} className="lg:hidden p-1.5 rounded-lg hover:bg-zinc-100 text-zinc-400">
            <X size={16} />
          </button>
        </div>
        ...
        <nav className="flex-1 px-3 py-4 overflow-y-auto">
          ...
          <div className="space-y-0.5">
            {nav.map(n => (
              <NavLink key={n.to} to={n.to} onClick={onClose} className={({isActive}) => `sidebar-link ${isActive?'active':''}`}>
