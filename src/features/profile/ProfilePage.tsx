...
import { User, Mail, Lock, LogOut, Shield, Camera, RefreshCw, KeyRound } from 'lucide-react';
...
export default function ProfilePage() {
  ...
  return (
    <div className="space-y-6 animate-slide-up max-w-xl">
      ...
      <div className="luxury-card p-6 flex items-center gap-5 card-rise">
        <div onClick={() => !uploading && fileInputRef.current?.click()}
          className="relative w-16 h-16 rounded-2xl cursor-pointer group flex-shrink-0">
          {user?.avatar ? (
            <img src={user.avatar} className="w-16 h-16 rounded-2xl object-cover" alt="Profile" />
          ) : (
          ...
          <div className="absolute inset-0 bg-black/40 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
            {uploading ? <RefreshCw size={16} color="white" className="animate-spin"/> : <Camera size={16} color="white"/>}
          </div>
      ...
      <div className="luxury-card p-6 card-rise" style={{animationDelay:'80ms'}}>
      ...
      <div className="luxury-card p-6 card-rise" style={{animationDelay:'140ms'}}>
        <div className="flex items-center gap-2 mb-5">
      ...
      <div className="luxury-card p-6 border-red-100 card-rise" style={{animationDelay:'200ms'}}>
